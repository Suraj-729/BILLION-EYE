      ...
      case "AssignedEvents":
        return dashboardData.filter(
          (event) =>
            ...
            event.assignedTo === agencyId && // 👈 Only this agency
            event.ground_staff // 👈 Only if ground staff is assigned
        );
  
      case "ResolvedEvents":
        return dashboardData.filter(
          (event) =>
            (event.status === "closed" || event.status === "Rejected") &&
            event.assignedTo === agencyId // Optional: Filter by agency if needed
        );
